  ...
  return (
    <>
      <nav className="nav-container fixed top-0 left-0 right-0 z-[100] p-3 md:p-6 flex justify-center items-start pointer-events-none">
        <div className="w-full max-w-6xl bg-background/95 border border-border px-5 md:px-8 py-3 md:py-4 rounded-[2rem] md:rounded-[3rem] backdrop-blur-2xl shadow-2xl flex items-center justify-between pointer-events-auto transition-all duration-500 hover:border-foreground/20">

          {/* Brand Area */}
          <Link href="/" className="flex items-center gap-4 group">
            <div className="flex flex-col">
              <span className="font-serif italic text-lg md:text-2xl font-light tracking-tighter leading-none group-hover:tracking-normal transition-all duration-700">Arslan Javed</span>
            ...
            <div className="hidden lg:flex items-center gap-1 border-r border-border/60 pr-6 mr-1">
              {navItems.map((item) => (
                <Link
                  key={item.name}
                  href={item.href}
                  className="px-4 py-2 text-[9px] uppercase font-black tracking-[0.25em] hover:text-foreground/40 transition-colors"
                >
                  {item.name}
                </Link>
              ))}
      ...
      >
        <button
          ...
          className="absolute top-8 right-8 w-14 h-14 bg-foreground text-background flex items-center justify-center rounded-full active:scale-90 transition-transform shadow-2xl z-[201]"
        >
          <X className="w-6 h-6" />
        </button>
        ...
        <div className="flex flex-col h-full relative z-10">
          <div className="space-y-6" ref={linksRef}>
            <p className="text-[10px] font-mono uppercase tracking-[0.5em] text-secondary mb-12 opacity-40">Navigation Hub</p>
            {navItems.map((item) => (
              <Link
                key={item.name}
                href={item.href}
                onClick={() => setIsOpen(false)}
                className="mobile-link group flex items-end justify-between text-5xl font-black tracking-tighter py-2"
              >
                <span className="group-hover:translate-x-6 transition-transform duration-500 uppercase">{item.name}</span>
                <ArrowUpRight className="w-10 h-10 opacity-20 group-hover:opacity-100 group-hover:rotate-45 transition-all duration-500" />
              </Link>
            ))}
          </div>
          ...
          <div className="mt-auto pb-12 space-y-10">
            ...
            <div className="flex justify-between items-center text-[10px] font-mono text-secondary uppercase tracking-[0.3em] font-bold">
